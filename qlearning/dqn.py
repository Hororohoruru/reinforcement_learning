...
class ModifiedTensorBoard(TensorBoard):
    ...
    # Overriding init to set initial step and writer (we want one log
    # file for all .fit() calls)
...
class DQNAgent:
    def __init__(self):
        # Main model -> what we train
        self.model = self.create_model()

        # Target model -> what we predict
    ...
    def get_qs(self, state):
        return self.model.predict(np.array(state).reshape(-1,
                                                          *state.shape)/255)[0]
